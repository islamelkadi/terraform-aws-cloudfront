...
### Complete Example - With Logging and Custom Behaviors
...
module "cloudfront" {
  ...
  # Custom cache behavior for API paths
  ...
  logging_prefix = "cloudfront/dashboard/"
...
module "cloudfront" {
  ...
  # S3 website endpoint (for index.html support in subdirectories)
  origin_domain_name       = "my-bucket.s3-website-us-east-1.amazonaws.com"
  use_s3_website_endpoint  = true
...
**Recommended**: Use Origin Access Control (OAC) - the modern approach:
```hcl
use_origin_access_control = true
```

**Legacy**: Origin Access Identity (OAI) - deprecated by AWS:
```hcl
use_origin_access_control = false
origin_access_identity_path = "origin-access-identity/cloudfront/ABCDEFG1234567"
```
...
The default configuration includes SPA-friendly error responses that redirect 404/403 errors to index.html:
